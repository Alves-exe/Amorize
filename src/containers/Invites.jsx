...
import { useAuth } from "../services/AuthContext"; // ou de onde vem seu currentUser
...
export default function Invites() {
  ...
  useEffect(() => {
    ...
    // Query para pegar só convidados do usuário logado
    const convidadosRef = collection(db, "convidados");
    const q = query(convidadosRef, where("userId", "==", currentUser.uid));
  ...
  const adicionarConvidado = async () => {
    ...
    await addDoc(collection(db, "convidados"), {
      ...
      confirmado: false,
      userId: currentUser.uid, // importante: associa convidado ao usuário logado
    });
  ...
  if (!currentUser) {
    return <p>Por favor, faça login para gerenciar seus convidados.</p>;
  }
